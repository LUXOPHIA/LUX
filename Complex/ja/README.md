...
`LUX.Complex.CP1` は斉次複素座標 `[Z:W]` を保持する `TSingleCP1` / `TDoubleCP1` を提供する。値は `Z / W` であり、非零の `Z` と `W = 0` の組が無限遠点を表す。ユニット検索パスに LUX のルートと `Complex` ディレクトリを追加し、`LUX.Complex.CP1` を uses に指定する。例えば CP1 型の変数で `Z := Pow2( Z ) + C` と書けば二次反復になる。依存先は `LUX.Complex` と RTL であり、描画処理には依存しない。

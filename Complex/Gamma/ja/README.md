...
`Gamma*` は `A * Exp(Ln(B) * (z - 1/2) - B + Ln(2*pi)/2)` を計算する。`LnGamma*` は対数形の各項を計算する。`N` ごとに係数列と `g` の組が決まっており、`N` を増やすだけで浮動小数点演算の誤差が必ず小さくなるとは限らない [2]。
...
双対数版では、共役や絶対値の二乗を含め、実部・虚部を通じて微分を伝播する。これらの中間操作自体は正則ではなく、二つの実成分に対する演算として微分される。
